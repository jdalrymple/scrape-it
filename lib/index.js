...
scrapeIt.scrapeHTML = ($, opts) => {
  ...
  let handleDataObj = async (data, $context) => {
    let pageData = {};
    ...
    for (let [key, value] of Object.entries(data)) {
      ...
      let $elm = value.selector ? $(value.selector, $context) : $context;
      ...
      if (value.listItem) {
        ...
        for (let i = 0; i < $items.length; ++i) {
          ...
          const convert = value.convert || function(x) { return x };
        ...
        // Handle closest
        if (value.closest) $elm = $elm.closest(value.closest);
        ...
        let key = typpy(value.how, Function) ? value.how($elm) : $elm[value.how]();
        key = key === undefined ? '' : key;

        if (value.trimValue && typpy(key, String)) {
          key = key.trim();
        }

        if (value.convert) {
          key = value.convert(key, $elm);
        }
